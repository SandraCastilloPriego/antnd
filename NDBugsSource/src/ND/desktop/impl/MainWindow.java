...
public class MainWindow extends JFrame implements NDModule, Desktop,
    ...
    private File parametersReportPath;
    ...
    @Override
    public void saveParameterReportToXML(Element xmlElement) {
        Document parentDocument = xmlElement.getOwnerDocument();
        Element paramElement = parentDocument.createElement("parameter");
        paramElement.setAttribute("name", "Report paths");
        if (this.parametersReportPath == null) {
            return;
        }
        paramElement.setTextContent(this.parametersReportPath.getAbsolutePath());
        xmlElement.appendChild(paramElement);
    }

    @Override
    public void loadParameterReportFromXML(Element xmlElement) {
        NodeList list = xmlElement.getElementsByTagName("parameter");
        for (int i = 0; i < list.getLength(); i++) {
            Element nextElement = (Element) list.item(i);
            String paramName = nextElement.getAttribute("name");
            if ("Report paths".equals(paramName)) {
                try {
                   ...
                   this.parametersReportPath = new File(fileString.trim());
                } catch (Exception e) {
                }
            }
        }
    ...
    public void setParameteresReport(File path) {
        this.parametersReportPath = path;
    }

    @Override
    public File getParameteresReport() {
        return parametersReportPath;
    }

    @Override
    public Dataset getParentDataset(String parent) {
        Dataset[] data = NDCore.getDesktop().getAllDataFiles();
        for(Dataset d : data){
            if(d.getDatasetName().equals(parent)){
                return d;
            }
        }
        return null;
