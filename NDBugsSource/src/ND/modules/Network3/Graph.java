...
public class Graph {
        ...
        public void addAnt(String string) {
                try {
                        List<Node> n = this.getNodesWithSpecie(string);
                      //  Node randomNode = n.get(this.rand.nextInt(n));
                       // randomNode.augmentAnts();
                } catch (NullPointerException e) {
                        e.printStackTrace();
                }
        }

        private List<Node> getNodesWithSpecie(String string) {
                List<Node> nodes = new ArrayList<>();
                for(Node n : this.nodes){
                        List<Species> species= n.getPossibleEdges();
                        if(species.size() > 0){
                                nodes.add(n);
                        } 
                }
