...
public class LoopAnalyzerModule implements NDProcessingModule {
        ...
        public Task[] runModule(ParameterSet parameters) {
                ...
                Task tasks[] = new LoopAnalyzerTask[1];
                if (NDCore.getDesktop().getSelectedDataFiles().length == 0) {
                        NDCore.getDesktop().displayErrorMessage("You need to select a metabolic model.");
                } else {
                        tasks[0] = new LoopAnalyzerTask((SimpleBasicDataset) NDCore.getDesktop().getSelectedDataFiles()[0], (SimpleParameterSet) parameters);
